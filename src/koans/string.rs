...
fn strings_with_strs() {
    ...
    assert!(hello + world == "Hello World".to_string());
}
...
fn using_chars() {
    ...
    assert!(string.chars().nth(2) == "yeah".chars().nth(0));
}
...
fn slicing_the_string() {
    let string = "Boom";
    assert!(&string[0..2] == "Bo"); // どうやら0以上2未満らしい
}
...
fn insert_and_remove_into_string() {
    let mut string = String::from("Hello");
    string.insert(2, 'e');
    assert!(string == "Heello".to_string());

    string.remove(2);
    assert!(string == "Hello".to_string());
}
